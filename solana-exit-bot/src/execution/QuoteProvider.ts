...
export class JupiterQuoteProvider implements QuoteProvider {
  ...
  constructor(
    ...
    private readonly options: { timeoutMs?: number; retries?: number; cacheMs?: number } = {}
  ) {}
  ...
  async getQuote(params: QuoteRequest): Promise<Quote> {
    ...
    for (let attempt = 0; attempt < attempts; attempt += 1) {
      try {
        ...
        let res: Response;
        try {
          res = await fetch(`${this.endpoint}?${key}`, { headers, signal: controller.signal });
        } finally {
  ...
  async quoteForPosition(position: Position, outputMint: string, sellPct: number, slippageBps: number): Promise<Quote> {
    ...
    const amountRaw = (baseRaw * BigInt(Math.round(remainingFraction * sellFraction * 1_000_000))) / 1_000_000n;
