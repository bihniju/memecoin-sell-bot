...
export interface Position {
  ...
  entryTimestamp: number; currentPrice: number; highestPrice: number; lowestPrice: number; realizedPnL: number; unrealizedPnL: number;
  remainingPercentage: number; sellState: SellState; lastTrigger?: ExitTrigger; lastSellAttempt?: number; sellSignature?: string; completedTakeProfitLevels: Set<string>;
}
export interface PriceTick { mint: string; price: number; timestamp: number; volumeBuy?: number; volumeSell?: number; priceImpactBps?: number; slot?: number; source?: string; }
...
export interface RiskConfig {
  ...
  fallingWeightLowerTicks: number; fallingWeightAcceleration: number; fallingWeightVolumeImbalance: number; fallingWeightLiquidity: number;
  trailingStopEnabled: boolean; trailingActivationPct: number; trailingStopPct: number; takeProfitEnabled: boolean;
  takeProfitLevels: TakeProfitLevel[]; maxPriceImpactBps: number; decisionCooldownMs: number;
...
export interface MarketConfig { outputMint: string; quoteApiUrl: string; swapApiUrl: string; jupiterApiKey?: string; websocketProvider: "solana" | "helius"; sampleDebounceMs: number; heartbeatMs: number; }
...
export interface ExitLatencyTimestamps { marketEventAt: number; riskDecisionAt: number; quoteRequestedAt: number; quoteReceivedAt: number; transactionBuiltAt: number; transactionSignedAt?: number; transactionSubmittedAt?: number; confirmationAt?: number; }
