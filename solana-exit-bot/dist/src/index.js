...
const sellExecutor = new SellExecutor(config, positions, new StaticQuoteProvider(), new TransactionBuilder(), new RetryManager(config.execution), new PriorityFeeManager(config.execution), new InMemoryTransport(), logger);
...
priceMonitor.on("tick", (tick) => {
    ...
    if (decision) {
        ...
        if (decision.trigger === "TAKE_PROFIT") {
            ...
            if (level)
                positions.markTakeProfitCompleted(tick.mint, level.id);
        }
