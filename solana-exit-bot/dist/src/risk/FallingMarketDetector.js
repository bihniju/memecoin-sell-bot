...
export class FallingMarketDetector {
    ...
    evaluate(prices, liquidity) {
        ...
        let score = 0;
        if (shortDropPct >= this.config.fallingDropPct)
            score += this.config.fallingWeightPriceDrop;
        if (consecutiveLowerTicks >= this.config.consecutiveLowerTicks)
            score += this.config.fallingWeightLowerTicks;
        if (acceleratingDecline)
            score += this.config.fallingWeightAcceleration;
